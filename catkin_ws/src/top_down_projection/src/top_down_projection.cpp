...
void FeatureExtractor::constructLane(){
  ...
  ROS_INFO_STREAM("All line size before activeLaneSeg: "<<allLines.size());  
...
void FeatureExtractor::createLaneletMap(){
  //Create laneletmap
      ...
      if(bg::length(odomLS) > odomLineLength){
        ...
        for(size_t j=0; j < allLineStrings.size(); j++){
          ...
          if(bg::length(line) > lineLength){
            ...
            auto point3d1 = lanelet::Point3d(lanelet::utils::getId(), p1.get<0>(), p1.get<1>(), 0);
            lanelet::BasicPoint3d pProj1 = lanelet::geometry::project(line3d, point3d1);
            ...
            auto M1 = std::get<0>(projLineDetails);
            auto gradDiff1 = std::abs(M2-M1);
            tempXVec.clear(); tempXVec.push_back(p2.get<0>()); tempXVec.push_back(pProj2.x());
            tempYVec.clear(); tempYVec.push_back(p2.get<1>()); tempYVec.push_back(pProj2.y());
       ...
       if(selectedLines.size() > 0){
          ...
          for(size_t k=0; k<selectedLines.size(); k++){
            auto checkPair = selectedLines[k];
            if(std::find(eraseVec.begin(), eraseVec.end(), k) != eraseVec.end())
              continue;
            auto line1 = allLineStrings[checkPair.first];
            auto pProj1 = checkPair.second.first;auto pProj2 = checkPair.second.second;   
            for(size_t l=0; l<selectedLines.size(); l++){
              auto pair = selectedLines[l];
              if(std::find(eraseVec.begin(), eraseVec.end(), l) != eraseVec.end() || k == l)
                continue;
              auto line2 = allLineStrings[pair.first];
              auto pProj3 = pair.second.first;auto pProj4 = pair.second.second;   
              auto xdiff1 = std::abs(pProj3.x()- pProj1.x()); 
              auto ydiff1 = std::abs(pProj3.y()- pProj1.y());
              auto xdiff2 = std::abs(pProj4.x()- pProj2.x()); 
              auto ydiff2 = std::abs(pProj4.y()- pProj2.y()); 
              if(xdiff1 < 0.1 && ydiff1 < 0.1 && xdiff2 < 0.1 && ydiff2 < 0.1){
                auto selected = k;
          ...
          if(orderVec.size() == 1){
            ...
            if(isLeft(lanelet::Point3d(lanelet::utils::getId(), p1.get<0>(), p1.get<1>(), 0), lanelet::Point3d(lanelet::utils::getId(), p2.get<0>(), p2.get<1>(), 0), lanelet::Point3d(lanelet::utils::getId(), x, y, 0)))
              left = true;
            lanelet::LineString3d checkLine3d(lanelet::utils::getId(), {lanelet::Point3d(lanelet::utils::getId(), x1, y1, 0), lanelet::Point3d(lanelet::utils::getId(), x2, y2, 0)});
            //Construct a new arbitary line 
            // if the line is on the right of the odom line, then construct the
            // arbitary line on the left. We need a logic to find connection
            // to this arbitary line from the previous and next lines that are
            // in similar position
            double dist = 3.;
            if(!left)
              dist *= -1;
            ...
            auto tempP3 = lanelet::Point3d(lanelet::utils::getId(), newLine2D[0].x(), newLine2D[0].y(), 0);
            auto tempP4 = lanelet::Point3d(lanelet::utils::getId(), newLine2D[1].x(), newLine2D[1].y(), 0);
            lanelet::LineString3d ls3d2(lanelet::utils::getId(), {tempP3, tempP4});
            lanelet::Lanelet lanelet(lanelet::utils::getId(), ls3d1, ls3d2);
  ...
  for(size_t i=0; i<lineStrings.size(); i++){
    ...
    if(d1 < 10 && listInter.size() == 1){
      ...
      /*if(listInter.size() == 1){
        ...
        auto d2 = bg::length(linestring2);
        if(d1 < d2){
          auto line3d1 = linestringToLineString3d(linestring1);
          auto line3d2 = linestringToLineString3d(linestring2);
          auto lsBox1 = lanelet::geometry::boundingBox3d(line3d1);
          auto lsBox2 = lanelet::geometry::boundingBox3d(line3d2);
          if(lanelet::geometry::intersects(lsBox1, lsBox2)){
            ROS_INFO_STREAM("There are: "<<listInter.size()<<" "<<d1<<" "<<d2);
 ...
 /* for(auto& lineSeg: lines){
  ...
  /*for(auto& lineSegVec: allLines){
    ...
    for(auto& takeSeg: lineSegVec){
      ...
      cv::line(output_image, p1, p2, color, thickness, cv::LINE_8);
      if(first){
        cv::circle(output_image, cv::Point(value_y1, value_x1), 2., odom_colour1, CV_FILLED);
        first = false;
      }
...
void FeatureExtractor::joinLaneSegment(std::tuple<Segment,double,double> newSeg, std::tuple<Segment,double,double> odomSeg){
  ...
  if(activeLaneSeg.size() == 0){
    ...
    for(size_t i=0; i<activeLaneSeg.size(); i++){
      ...
      if(slopeDiff < 0.05 && y_diff1 < 0.25 && y_diff1 < smallD){ //0.05 works
        found = true;
        smallD = y_diff1;
        smallIndex = i;
        //break;
...
void FeatureExtractor::joinLinesFurther(std::vector<std::pair<std::tuple<Segment,double,double>, std::tuple<Segment,double,double>>> newLine){
  ...
  if(activeLaneSeg2.size() == 0){
    ...
    for(size_t i=0; i<activeLaneSeg2.size(); i++){
      ...
      auto y_diff2 = std::abs(std::abs(y)-std::abs(y1));
      float d = std::sqrt(std::pow((x4-x1),2)+std::pow((y4-y1),2));
      if(slopeDiff < 0.07 && y_diff1 < 0.5 && y_diff1 < smallD){ //0.05 works
        found = true;
        smallD = y_diff1;
        smallIndex = i;
        //break;
    ...
    for(size_t i=0; i<eraseVec.size(); i++){
      activeLaneSeg2.erase(activeLaneSeg2.begin()+eraseVec[i]);
    }
